...
## 버전
Version 2.6.0, August 08, 2026 00:00:00
...
## 선행 요건 확인
Chzzk Video Manager의 모든 기능을 활용하려면, Chzzk Video Downloader와 Chzzk Transport Finalizer가 올바르게 설정되어 있어야 하며, Chzzk Video Downloader를 실행하려면 Streamlink(버전 7.0.0 이상)와 FFmpeg의 공식 메이저 버전(버전 7.0 이상)이 올바르게 설정되어 있어야 합니다. Chzzk Video Manager가 처음 실행될 때 이와 같은 선행 요건을 모두 확인하는 과정을 거치며, 이를 만족하지 못할 경우 다음 그림과 같은 대화 상자가 표시됩니다.
...
## 비디오 추가
다운로드할 비디오를 추가하려면 ☰ 아이콘을 클릭한 후 `비디오 추가` 항목을 선택하면 비디오 추가 대화 상자가 열립니다. 추가할 비디오 번호 또는 비디오 URL을 입력하고 `확인` 버튼을 누릅니다.
...
## 비디오 삭제
비디오를 삭제하려면, 목록에서 삭제할 비디오를 선택한 다음 ☰ 아이콘을 클릭한 후 `비디오 삭제` 항목을 선택합니다. 비디오 삭제 여부를 확인하는 대화 상자에서 `OK` 버튼을 누르면 비디오가 삭제됩니다.
...
## 다운로드 시작
다운로드를 시작하려면, ☰ 아이콘을 클릭한 후 `다운로드 시작` 항목을 선택합니다.

## 다운로드 중지
다운로드를 중지하려면, ☰ 아이콘을 클릭한 후 `다운로드 중지` 항목을 선택합니다.
...
## 다운로드가 완료된 비디오 정리
다운로드가 완료된 비디오를 목록에서 정리하려면, ☰ 아이콘을 클릭한 후 `다운로드 완료 항목 정리` 항목을 선택합니다.

<div style='text-align: center'>
<img src='../../img/screenshots/vman_ko-KR/vman_clear_completed.png' />
<p><i>(이 이미지는 최신 정보와 다를 수 있습니다.)</i></p>
</div>
...
## 비디오 등록 정보 확인
비디오의 등록 정보를 확인하려면, 목록에서 등록 정보를 확인할 비디오를 선택한 다음 ☰ 아이콘을 클릭한 후 `등록 정보...` 항목을 선택합니다.
...
목록에 비디오가 하나 이상 존재하는 상태에서 ☰ 아이콘을 클릭한 후 `비디오 목록 저장...` 항목을 선택하면, 파일을 저장할 디렉터리와 이름을 변경할 수 있는 대화 상자가 열립니다.

## 비디오 목록 불러오기
매번 비디오를 추가하는 대신 저장했던 비디오 목록을 불러와서 추가할 수 있습니다.

☰ 아이콘을 클릭한 후 `비디오 목록 불러오기...` 항목을 선택하면, 비디오 목록 파일을 선택할 수 있는 대화 상자가 열립니다.

불러 온 목록에 있는 비디오가 아미 현재 목록에 있더라도 자동으로 인식되어 처리됩니다.

## 비디오 목록 가져오기
바이너리 형식의 목록 파일 대신 텍스트 형식으로 저장된 비디오 번호 목록을 가져올 수 있습니다.

☰ 아이콘을 클릭한 후 `가져오기...` 항목을 선택하면, 텍스트 파일을 선택할 수 있는 대화 상자가 열립니다.

불러 온 목록에 있는 비디오가 아미 현재 목록에 있더라도 자동으로 인식되어 처리됩니다.

텍스트 파일의 인코딩은 UTF-8이며 형식은 다음과 같습니다.
...
## 비디오 목록 내보내기
비디오 목록을 직접 외부에서 편집하거나 전달하기 위해 텍스트 형식으로 내보낼 수 있습니다.

☰ 아이콘을 클릭한 후 `내보내기...` 항목을 선택하면, 텍스트 파일을 선택할 수 있는 대화 상자가 열립니다.

## 목록 갱신
목록을 즉시 갱신하려면, ☰ 아이콘을 클릭한 후 `새로 고치기` 항목을 선택합니다.

## 화면 색상 테마 모드 전환하기
Chzzk Video Manager는 기본적으로 시스템의 앱 색상 테마 설정에 따라 자동으로 화면 색상이 변경됩니다.
하지만 필요에 따라 시스템 설정과 관계없이 앱의 색상 테마를 직접 지정할 수도 있습니다.

- ☰ 아이콘을 클릭한 후 `설정...`항목을 선택해 설정 대화 상자를 엽니다.
- `기능` 탭에서 `색상 테마 모드` 항목을 찾습니다.
- `밝은 테마` 또는 `어두운 테마` 중 원하는 모드를 선택합니다.

## 도구 환경 열기
도구 환경을 열려면, ☰ 아이콘을 클릭한 후 `도구` 항목에서 `도구 환경 (Command Prompt)` 또는 `도구 환경 (PowerShell)` 항목을 선택합니다.

## 저장 디렉터리 열기
다운로드한 비디오가 저장된 디렉터리를 열려면, ☰ 아이콘을 클릭한 후 `도구` 항목에서 `저장 디렉터리 열기` 항목을 선택합니다.

## 손상된 파일 제거하기
다운로드 도중에 네트워크 연결이 끊어지거나 시스템이 종료되는 등 비정상 상황이 발생하면, 임시 디렉터리에 손상된 파일이 남을 수 있습니다.
남아 있는 손상된 파일을 제거하려면, ☰ 아이콘을 클릭한 후 `도구` 항목에서 `손상된 파일 제거` 항목을 선택합니다.

<div style='text-align: center'>
<img src='../../img/screenshots/vman_ko-KR/vman_purge_broken.png' />
<p><i>(이 이미지는 최신 정보와 다를 수 있습니다.)</i></p>
</div>

## 다운로드 도구 설정 보기
Chzzk Video Downloader의 설정을 보려면, ☰ 아이콘을 클릭한 후 `도구` 항목에서 `설정 보기...` 항목을 선택합니다.

<div style='text-align: center'>
<img src='../../img/screenshots/vman_ko-KR/vman_configuration.png' />
<p><i>(이 이미지는 최신 정보와 다를 수 있습니다.)</i></p>
</div>
...
Chzzk Transport Finalizer를 재시작하려면, ☰ 아이콘을 클릭한 후 `도구` 항목에서 `Chzzk Transport Finalizer 재시작` 항목을 선택합니다.

재시작 시점에 최종 처리 중인 파일이 있었다면 손상될 가능성이 있습니다. 이 때는 해당 파일을 삭제하면 자동으로 다시 최종 처리가 시작됩니다.

<div style='text-align: center'>
<img src='../../img/screenshots/vman_ko-KR/vman_restart_finalizer.png' />
<p><i>(이 이미지는 최신 정보와 다를 수 있습니다.)</i></p>
</div>
...
### 최종 처리 단계 사용자 정의 설정
다운로드 하는 파일의 VOD 형식이 `UPLOAD`일 경우, `최종 처리` 설정의 `사용자 정의 선택 사항`에 매개 변수를 지정하면, 최종 처리 시에 이 매개 변수가 `FFmpeg`에 전달됩니다. 예를 들어, 다음 설정은 `FFmpeg`을 사용하여 `HEVC` 코덱으로 인코딩하도록 설정합니다:
...
- **최신 버전 확인 주기** - Chzzk Video Manager를 시작할 때 최신 버전을 확인하는 주기를 설정합니다. `최신 버전 확인` 버튼을 누르면 즉시 최신 버전을 확인할 수 있습니다.
...
- **시스템 성능** - Chzzk Video Manager가 실행 중인 시스템의 성능을 지정합니다. 비디오를 다운로드할 때 시간 초과로 인한 오류가 발생한다면, 이 설정을 한 단계 아래로 낮추는 것을 고려해 볼 수 있습니다.
- **다시보기 정보를 기반으로 메타데이터 저장** - 다시보기 정보를 기반으로 메타데이터를 저장합니다.
...
<div style='text-align: center'>
<img src='../../img/screenshots/vman_ko-KR/vman_settings_tools.png' />
<p><i>(이 이미지는 최신 정보와 다를 수 있습니다.)</i></p>
</div>

- **비디오 도구: Streamlink** - Streamlink의 설치 여부 및 버전을 확인하거나 최신 버전을 다운로드합니다.
- **비디오 도구: FFmpeg** - FFmpeg의 설치 여부 및 버전을 확인하거나 최신 버전을 다운로드합니다.
- **알림 플러그인: 경로** - 알림 플러그인의 경로를 설정합니다.
- **알림 플러그인: 매개 변수** - 알림 플러그인의 매개 변수를 설정합니다.
- **알림 플러그인: 텍스트 형식** - 알림 플러그인에 전달할 텍스트의 형식을 설정합니다.
...
<div style='text-align: center'>
<img src='../../img/screenshots/vman_ko-KR/vman_settings_login.png' />
<p><i>(이 이미지는 최신 정보와 다를 수 있습니다.)</i></p>
</div>
