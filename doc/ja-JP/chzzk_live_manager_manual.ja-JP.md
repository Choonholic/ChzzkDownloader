...
## 必要条件の確認
Chzzk Live Managerが正常に機能するためには、Chzzk Live DownloaderとChzzk Transport Finalizerが正しく設定されている必要があります。また、Chzzk Live DownloaderはStreamlink（バージョン7.0.0またはそれ以上）とFFmpegの公式メジャーバージョン（バージョン7.0またはそれ以上）の設定が必要です。Chzzk Live Managerの起動時に、これらの必要条件が満たされているかチェックし、満たされていない場合は、以下のダイアログが表示されます。
...
読み込まれたリストに含まれるチャンネルは、すでに現在のリストに存在している場合でも自動的に認識され、処理されます。

## チャンネルリストの読み込み
バイナリ形式のリストファイルの代わりに、テキスト形式で保存されたチャンネルリストを読み込むことができます。

☰アイコンをクリックし、`インポート...`を選択すると、テキストファイルを選択するためのダイアログが表示されます。

読み込まれたリストに含まれるチャンネルは、すでに現在のリストに存在している場合でも自動的に認識され、処理されます。

テキストファイルの文字エンコーディングはUTF-8で、形式は以下の通りです。
...
## チャンネルリストの書き出し
チャンネルリストを外部で編集したり、他者と共有したりするために、テキスト形式で書き出すことができます。

☰アイコンをクリックし、`エクスポート...`を選択すると、テキストファイルを指定するためのダイアログが表示されます。

## カラーテーマモードの切り替え
Chzzk Live Managerは、通常はシステムのアプリカラーテーマ設定に従って自動的に画面の色が切り替わります。
ただし、必要に応じてシステム設定とは別に、アプリのカラーテーマを直接指定することもできます。

- ☰アイコンをクリックし、`設定...`を選択して設定ダイアログを開きます。
- `機能`の`カラーテーマモード`項目を確認します。
- `ライト`または`ダーク`を選択します。
...
- **更新確認の間隔** - Chzzk Live Managerの起動時に最新バージョンを確認する間隔を設定します。`アップデート確認`ボタンを押すと、すぐに最新バージョンを確認することもできます。
- **起動時にログイン状態を通知** - 起動時にログイン状態を確認し、ログインしていない場合に通知を表示するかどうかを設定します。ゲストとして使用する場合は、この設定をオフにしてください。
...
- **ハートビートを受信できない場合、チャンネルを自動で復旧** - チャンネルのハートビートを受信できなかった場合に、自動でチャンネル復旧を試行するかを設定します。接続が失われた時点でダウンロードが進行中だった場合、その時点で終了し、再度ダウンロードが開始されます。
- **空き容量が次の値を下回る場合ダウンロード停止** - ディスク容量不足時の停止しきい値を、％単位またはサイズ単位で設定します。サイズ単位で設定する場合は、SI 単位（KB、MB、GB...）や IEC 単位（KiB、MiB、GiB...）を使用できるほか、接頭辞のみ（K、Ki、M、Mi、G、Gi...）を指定することも可能です。もちろん、単位を付けずにバイト単位で指定することもできます。
- **スリープモード** - Chzzk Live Managerが動作している間のシステムのスリープモードを設定します。
- **システムパフォーマンス** - Chzzk Live Managerが現在実行されているシステムのパフォーマンスを指定します。チャンネル追加や更新時にタイムアウトによるエラーが発生する場合、パフォーマンスレベルを1段階下げて再試行してください。
...
- **JSON-RPCサーバー: 外部からのリモート接続を許可** - ダウンローダーを実行しているPCの外部から、リモートでJSON-RPCサーバーにアクセスすることを許可します。
- **JSON-RPCサーバー: ベースポート** - JSON-RPCサーバーのベースポート番号を設定します。実際のチャンネルごとのポート番号は、ベースポートとチャンネルIDを組み合わせた値で指定されます。例えば、ベースポートが`62000`でチャンネルIDが`3`の場合、実際のポート番号は`62003`に設定されます。
...
- **JSON-RPCサーバー: 外部からのリモート接続を許可** - Chzzk Transport Finalizerを実行しているPCの外部から、リモートでJSON-RPCサーバーにアクセスすることを許可します。
- **JSON-RPCサーバー: ポート** - Chzzk Transport FinalizerのJSON-RPCサーバーのポート番号を設定します。
- **JSON-RPCサーバー: ID** - Chzzk Transport FinalizerのJSON-RPCサーバーのIDを設定します。
- **最終処理方法** - 最終処理方法を設定します。
...
<div style='text-align: center'>
<img src='../../img/screenshots/lman_ja-JP/lman_settings_tools.png' />
<p><i>(この画像は最新の情報と異なる場合があります。)</i></p>
</div>

- **ビデオツール: Streamlink** - Streamlinkのインストール有無およびバージョンを確認するか、最新バージョンをダウンロードします。
- **ビデオツール: FFmpeg** - FFmpegのインストール有無およびバージョンを確認するか、最新バージョンをダウンロードします。
- **通知プラグイン: パス** - 通知プラグインのパスを設定します。
- **通知プラグイン: パラメーター** - 通知プラグインのパラメーターを設定します。
- **通知プラグイン: テキスト形式** - 通知プラグインのテキスト形式を設定します。
...
- **検証** - 設定した認証情報が正しいか確認し、ユーザー情報を表示します。

<div style='text-align: center'>
<img src='../../img/screenshots/lman_ja-JP/lman_settings_login.png' />
<p><i>(この画像は最新の情報と異なる場合があります。)</i></p>
</div>
