...
Chzzk Clip Manager와 Chzzk Clip Downloader가 같은 디렉터리에 설치되어 있으면 실행 시 자동으로 인식히며, 그렇지 않으면 **선행 요건 확인하기** 항목의 내용을 따라 Chzzk Clip Downloader의 위치를 지정해 주어야 합니다.
...
## 클립 추가
다운로드할 클립을 추가하려면 ☰ 아이콘을 누르고 `클립 추가하기` 항목을 선택하면 클립 추가하기 대화 상자가 열립니다. 추가할 클립 UID 또는 클립 URL을 입력하고 `확인` 버튼을 누릅니다.
...
## 클립 삭제
클립을 삭제하려면, 목록에서 삭제할 클립을 선택한 다음 ☰ 아이콘을 누르고 `클립 삭제하기` 항목을 선택합니다. 클립 삭제 여부를 확인하는 대화 상자에서 `OK` 버튼을 누르면 클립이 삭제됩니다.
...
목록에 클립이 하나 이상 존재하는 상태에서 ☰ 아이콘을 누르고 `클립 목록 저장하기...` 항목을 선택하면, 파일을 저장할 디렉터리와 이름을 변경할 수 있는 대화 상자가 열립니다.
...
<div style='text-align: center'>
<img src='../../img/screenshots/cman_ko-KR/cman_environment.png' />
<p><i>(이 이미지는 최신 정보와 다를 수 있습니다.)</i></p>
</div>
...
## 최소화 시 트레이로 이동
`기능` 설정에서 `최소화 시 트레이로 이동하기` 항목을 활성화하면, Chzzk Clip Manager가 최소화될 때 트레이로 이동합니다.
...
* **시작 시 Chzzk Clip Manager 최신 버전 확인하기** - Chzzk Clip Manager를 시작할 때 최신 버전 확인 여부를 결정합니다. 또는 `최신 버전 확인` 버튼을 눌러 수동으로 최신 버전을 확인할 수도 있습니다.
...
<div style='text-align: center'>
<img src='../../img/screenshots/cman_ko-KR/cman_settings_directory.png' />
<p><i>(이 이미지는 최신 정보와 다를 수 있습니다.)</i></p>
</div>
